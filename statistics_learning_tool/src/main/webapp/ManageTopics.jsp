...
<c:forEach items="${getTopics}" var="t">
	<tr>
		<td>${t.topic_ID}</td>
		<td>${t.topic_name}</td>
		<td>${t.topic_description}</td>
		<td>${t.topic_media_link}</td>
		<td>
			<form action="deleteTopic" method="post">
				<button class='deleteBtn btn btn-outline-danger' name="delete" value="${t.topic_ID}">Delete</button>
			</form>
		</td>
	</tr>
</c:forEach>
