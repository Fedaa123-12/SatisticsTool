...
<div class="table-responsive card-body mx-auto" style="max-width: 85%;">
	<table class="table">
		<tr>
			...
			<th>First Name</th>
			<th>Second Name</th>
			<th>Choose Class To ADD</th>
		</tr>
		
		<c:forEach items="${getStudents}" var="s">
			<tr>
				<td>${s.email}</td>
				<td>${s.username}</td>
				<td>${s.first_name}</td>
				...
				<td>
					<form action="addStudentToClass" method="post">
						...
						<input type="hidden" name="teacherEmail" value="${user_email}">
						<button class='updateBtn btn btn-outline-info'  name="addStudent" value="${s.email}">Add Student</button>
					</form>
				</td>
