...
<c:forEach items="${getMembers}" var="m">
	<tr>
		<td>${m.class_ID}</td>
		<td>${m.email}</td>
		<td>
			<form action="deleteClassMember" method="post">
				<input type="hidden" name="class_ID" value = "${m.class_ID}">
				<button class='deleteBtn btn btn-outline-danger' name="delete" value="${m.email}">Delete</button>
			</form>
		</td>
	</tr>
</c:forEach>
